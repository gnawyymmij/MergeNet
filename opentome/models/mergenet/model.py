...
MERGENET_SMALL_CANONICAL_KWARGS = {
    # Paper default: MN-L2 with geometry-faithful DTEM routing on the
    # original 2-D patch grid.  Keep this in sync with
    # configs/mergenet_l2_spatial_r3.yaml and tests/test_release_contract.py.
    "img_size": 224,
    "patch_size": 8,
    "local_depth": 6,
    "latent_depth": 6,
...
def _with_mergenet_small_defaults(kwargs):
    """Canonical single-branch MN-L2-R3 true-grid configuration."""
    resolved = dict(kwargs)
    explicit_keys = set(resolved)
    for key, value in MERGENET_SMALL_CANONICAL_KWARGS.items():
        # Absence selects the paper default.  Explicit ``None`` is meaningful
        # for controlled global/flat routing configs (no spatial radius) and
        # must not be silently rewritten to R3.  timm removes None-valued
        # kwargs before calling the factory, so an explicitly supplied DTEM
        # window is also the durable sentinel for global/flat geometry.
        if key not in resolved:
            if key == "dtem_spatial_radius" and "dtem_window_size" in explicit_keys:
                resolved[key] = None
            else:
                resolved[key] = value
    return resolved
...
class LocalEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=16, embed_dim=768, num_heads=12, mlp_ratio=4.0,
                 ...
                 dtem_spatial_backend: str = "dense"):
        super().__init__()
        ...
        self.dtem_spatial_backend = str(dtem_spatial_backend)
        if self.dtem_spatial_backend not in DTEM_SPATIAL_BACKENDS:
            raise ValueError(
                f"Unsupported dtem_spatial_backend {self.dtem_spatial_backend!r}; "
        ...
        if self.dtem_spatial_radius is None:
            ...
            spatial_neighbor_indices = torch.empty(0, 0, dtype=torch.long)
            spatial_neighbor_valid = torch.empty(0, 0, dtype=torch.bool)
        else:
            ...
            spatial_neighbor_indices, spatial_neighbor_valid = (
                adjacency_to_padded_neighbors(spatial_adjacency)
            )
        ...
        self.register_buffer(
            "_dtem_spatial_neighbor_indices",
            spatial_neighbor_indices,
            persistent=False,
        )
        self.register_buffer(
            "_dtem_spatial_neighbor_valid",
        ...
        self._tome_info = {
            ...
            "dtem_spatial_backend": self.dtem_spatial_backend,
            "patch_grid_size": patch_grid_size,
            "num_prefix_tokens": int(self.vit.num_prefix_tokens),
            "spatial_adjacency": None,
            "spatial_neighbor_indices": None,
            "spatial_neighbor_valid": None,
            "t": dtem_t,
    ...
    def forward(self, x):
        ...
        self._tome_info["spatial_neighbor_indices"] = (
            self._dtem_spatial_neighbor_indices
            if self._dtem_spatial_neighbor_indices.numel() > 0
            else None
        )
        self._tome_info["spatial_neighbor_valid"] = (
            self._dtem_spatial_neighbor_valid
            if self._dtem_spatial_neighbor_valid.numel() > 0
            else None
        )
...
class HybridToMeModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.dtem_spatial_backend = dtem_spatial_backend
...
def mergenet_small_cls(**kwargs):
    """Canonical MN-L2-R3 classifier with true 2-D Euclidean routing."""
