...
parser.add_argument('--patch_size', type=int, default=8, metavar='N',
                    help='Patch size for the paper models (default: 8)')
...
parser.add_argument(
    "--dtem_spatial_backend",
    type=str,
    default="dense",
    choices=["sparse", "dense"],
    help=(
        "Implementation for 2-D radius matching. sparse computes and transports "
        "only local-neighborhood edges; dense is the mathematical reference."
    ),
)
...
parser.add_argument(
    "--dtem_eval_grouping",
    type=str,
    default="alternating_per_layer_fast",
...
parser.add_argument('--branch_b_lambda_local', type=float, default=2.0,
...
parser.add_argument('--branch_b_dtem_window_size', type=int, default=None,
                    help='branch B DTEM window size; None follows the paper-default R3 geometry')
...
def _validate_shipped_model_options(args):
    """Refuse optional legacy modes not included in the public trainer."""
    if not _is_tome_family_model(args.model):
        return
    if args.pretrained:
        raise ValueError(
            "The public trainer does not include the historical MergeNet/ToMe "
...
def _parse_args():
    ...
    # Track the source, not value equality: explicit legacy/global/ablation
    # settings must never be overwritten by the paper-default R3 profile.
    ...
    dtem_window_explicit = (
        'dtem_window_size' in cfg
        or _option_was_provided(remaining, '--dtem_window_size')
    )
    dtem_radius_explicit = (
        'dtem_spatial_radius' in cfg
        or _option_was_provided(remaining, '--dtem_spatial_radius')
    )
    dtem_backend_explicit = (
        'dtem_spatial_backend' in cfg
        or _option_was_provided(remaining, '--dtem_spatial_backend')
    )
    img_size_explicit = (
        'img_size' in cfg
        or _option_was_provided(remaining, '--img_size')
    )
    local_cls_global_explicit = (
        'local_cls_global' in cfg
        or _option_was_provided(remaining, '--local_cls_global')
    )
    soft_topk_explicit = (
        'soft_topk' in cfg
        or _option_was_provided(remaining, '--soft_topk')
    )
    soft_topk_aux_weight_explicit = (
        'soft_topk_aux_weight' in cfg
        or _option_was_provided(remaining, '--soft_topk_aux_weight')
    )
    soft_topk_aux_start_explicit = (
        'soft_topk_aux_start_epoch' in cfg
        or _option_was_provided(remaining, '--soft_topk_aux_start_epoch')
    )
    soft_topk_aux_ramp_explicit = (
        'soft_topk_aux_ramp_epochs' in cfg
        or _option_was_provided(remaining, '--soft_topk_aux_ramp_epochs')
    )
    dtem_eval_grouping_explicit = (
        'dtem_eval_grouping' in cfg
        or _option_was_provided(remaining, '--dtem_eval_grouping')
    )
    ...
    if is_canonical_mergenet:
        if not img_size_explicit and args.img_size is None:
            args.img_size = 224
        if not dtem_window_explicit:
            args.dtem_window_size = 0
        if not dtem_radius_explicit and not dtem_window_explicit:
            args.dtem_spatial_radius = 3.0
        if not dtem_backend_explicit:
            args.dtem_spatial_backend = (
                'sparse' if args.dtem_spatial_radius is not None else 'dense'
            )
        ...
        if args.local_depth is None:
            args.local_depth = 6
        if args.latent_depth is None:
            args.latent_depth = 6
        if not local_cls_global_explicit:
            args.local_cls_global = True
        if not soft_topk_explicit:
            args.soft_topk = True
        if not soft_topk_aux_weight_explicit:
            args.soft_topk_aux_weight = 0.05
        if not soft_topk_aux_start_explicit:
            args.soft_topk_aux_start_epoch = 20
        if not soft_topk_aux_ramp_explicit:
            args.soft_topk_aux_ramp_epochs = 20
        if not dtem_eval_grouping_explicit:
            args.dtem_eval_grouping = 'alternating_per_layer_fast'
...
def _teacher_model_kwargs(args):
    ...
    if 'deit' in teacher_name and 'mergenet' not in teacher_name and 'hybridtome' not in teacher_name:
        ...
        kwargs.update({
            ...
            'dtem_spatial_backend': args.dtem_spatial_backend,
            'dtem_t': args.dtem_t,
